defmodule EyepatchTest do
  ...
  def request_hackney(method, uri, ip_address, protocol, connect_timeout, _pid)
  ...
  def connect_hackney_inet(), do: &connect_hackney(:head, &1, &2, :inet, &3, &4)
  def connect_hackney_inet6(), do: &connect_hackney(:head, &1, &2, :inet6, &3, &4)
  ...
  def connect_hackney(_method, uri, ip_address, protocol, connect_timeout, _pid) do
    ip_address =
      case :inet.ntoa(ip_address) do
        {:error, :einval} -> raise("Unable to parse ip address: #{inspect(ip_address)}")
        x -> x
